...
export function calculateDigitStats(ticks: TickData[], sampleWindow: number = 1000): DigitStats {
  ...
  // Fast path: only process new tick instead of full array
  ...
  // Find highest, 2nd highest, and lowest digits efficiently without full sort
  let highestDigit = 0, highestPct = percentages[0];
  let secondHighestDigit = 1, secondHighestPct = percentages[1];
  if (secondHighestPct > highestPct) {
    [highestDigit, highestPct, secondHighestDigit, secondHighestPct] = [1, secondHighestPct, 0, highestPct];
  }

  let lowestDigit = 0, lowestPct = percentages[0];
  ...
  const spread = highestPct - secondHighestPct;
  const rawConfidence = 60 + highestPct * 1.2 + spread * 1.5;
